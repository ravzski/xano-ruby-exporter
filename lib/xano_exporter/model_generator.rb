# frozen_string_literal: true

require 'fileutils'
require 'csv'
require 'active_support/inflector'

module XanoExporter
  class ModelGenerator
    attr_reader :csv_dir, :models_dir

    def initialize(csv_dir = 'csv_exports', models_dir = 'outputs/models')
      @csv_dir = csv_dir
      @models_dir = models_dir
    end

    def generate_models(csv_dir = 'csv_exports', models_dir = 'outputs/models')
      # Create the Xano directory inside models_dir
      xano_models_dir = File.join(models_dir, 'xano')
      FileUtils.mkdir_p(xano_models_dir) unless Dir.exist?(xano_models_dir)

      tables_data = load_tables_from_csv(csv_dir)

      # First, generate the module file to contain all models
      File.open(File.join(models_dir, 'xano.rb'), 'w') do |file|
        file.puts "# Generated by XanoExporter"
        file.puts "module Xano"
        file.puts "  # This module contains all models imported from Xano"
        file.puts "end"

        # Require all Xano model files
        tables_data.each do |table_data|
          file.puts "require_relative 'xano/#{table_data[:name].singularize}'"
        end
      end

      tables_data.each do |table_data|
        table_name = table_data[:name]
        model_name = table_name.singularize.camelize
        file_path = File.join(xano_models_dir, "#{table_name.singularize}.rb")

        File.open(file_path, 'w') do |file|
          file.puts "# Generated by XanoExporter"
          file.puts "module Xano"
          file.puts "  class #{model_name} < ApplicationRecord"
          file.puts "    self.table_name = 'xano__#{table_name}'"

          # Add any model validations or associations based on field types
          table_data[:headers].each do |header|
            next if %w[id created_at updated_at].include?(header)

            # Check if it's likely a foreign key
            if header.end_with?('_id')
              association_name = header.gsub(/_id$/, '')
              file.puts "    belongs_to :#{association_name}, optional: true, class_name: 'Xano::#{association_name.camelize}', foreign_key: '#{header}'"
            end
          end

          file.puts "  end"
          file.puts "end"
        end

        puts "Generated model: #{file_path}"
      end

      puts "Models generated in #{xano_models_dir}"
      puts "Main module file generated at #{File.join(models_dir, 'xano.rb')}"
      models_dir
    end

    private

    def load_tables_from_csv(csv_dir)
      tables_data = []

      Dir.glob("#{csv_dir}/*.csv").each do |csv_file|
        table_name = File.basename(csv_file, '.csv')

        begin
          headers = nil
          sample_data = nil

          CSV.foreach(csv_file, headers: true) do |row|
            headers = row.headers
            sample_data = row.to_h
            break # Only need the first row for structure
          end

          if headers
            tables_data << {
              name: table_name,
              headers: headers,
              sample_data: sample_data
            }
          end
        rescue => e
          puts "Error reading CSV file #{csv_file}: #{e.message}"
        end
      end

      tables_data
    end
  end
end